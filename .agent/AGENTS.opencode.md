# AGENTS.md — 企业开发者 AI Agent 配置（OpenCode 适配版）

> 本文件定义 AI Agent 在本项目中的工作方式。
> 适配平台：**OpenCode Agent**
>
> 加载方式：本文件由 `.opencode/AGENTS.md` 引导拦截加载。
---
...
> **注意：`subagent-driven-development` 不在列表中。** 作为原生的 OpenCode 主 Agent，你可以自动为专业化任务调度 subagents，或者使用 `@mention` 手动唤起 subagent。无需依赖受限的基础指令，直接结合自身能力与 `executing-plans` 推进即可。
...
> **注意：不使用 `subagent-driven-development`。** 使用 OpenCode 原生的 subagent 调度系统（如 `@reviewer`, `@tester` 或你自动按需创建的子代理）推进审查和任务分配更为精确。
...
在 OpenCode 环境中，你可利用自带的 subagents 机制（如 `@code-reviewer`）。在任务完成后请使用以下**自审/互审流程**：
