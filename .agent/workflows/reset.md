...
argument-hint: [可选：archive（归档已完成部分）/ discard（直接丢弃）]
---
...
1. **读取当前状态**：检查 `progress.md` 是否存在

   - 不存在 → 告知用户当前没有进行中的任务，无需清理，流程结束
   - 存在 → 读取内容，向用户展示当前任务概述和阶段

2. **向用户确认处理方式**（必须等待用户明确选择）：

   ```
   ⚠️ 发现进行中的任务：
     任务：[从 progress.md 读取的任务概述]
     当前阶段：[阶段] / 状态：[状态]
   
   请选择处理方式：
   
   [A] 归档（archive）
       → 将 progress.md / task_plan.md / findings.md
         复制到 .archive/tasks/YYYY-MM-DD-HH-MM/
       → 保留已调研的内容供日后参考
       → 清理工作区状态文件
   
   [B] 直接丢弃（discard）
       → 删除 progress.md / task_plan.md / findings.md
       → 完全清空，重新开始
   ```
...
3. **执行用户选择的操作**：
   ...
   **选 A — 归档**：
   ...
   2. 移动文件：progress.md → .archive/tasks/[时间戳]/
