...
- **关键记录**:
  ...
  - 当前批次: [第几批 / 共几批]
  - 人工检查点: [等待用户确认 ⬜]
  - **⛔ OpenSpec 同步规则（Path A 时强制执行，Path B 跳过此部分）**:
    - 执行清单来源: openspec/changes/[变更目录]/tasks.md
    - ⚠️ **每完成一个子任务，必须同步将 tasks.md 中对应条目的 `- [ ]` 更新为 `- [x]`**
    - tasks.md 的 checkbox 状态是 OpenSpec 的进度真相源，必须与 process.md 关键记录保持一致
    - 允许按批次统一更新（完成一个 Batch 后一次性更新该 Batch 所有条目），减少文件写入
    - ⛔ **严禁**只更新 process.md 而不更新 tasks.md
